...
class EUxfelTranslator(object):
    ...
    def __init__(self, state):
        ...
        if self._sel_module is None:
            ...
            for module_index in range(16):
                self._n2c[f"SQS_DET_DSSC1M-1/DET/{module_index}CH0:xtdf"] = ["photonPixelDetectors", "eventID"]
        else:
            self._n2c[f"SQS_DET_DSSC1M-1/DET/{self._sel_module}CH0:xtdf"] = ["photonPixelDetectors", "eventID"]
    ...
    def translate_core(self, evt, key):
        ...
        for k in self._c2n[key]:
            if k in evt:
                if key == 'eventID':
                    self._tr_event_id_sqs_dssc(values, evt[k])
                elif key == 'photonPixelDetectors':
                    self._tr_photon_detector_sqs_dssc(values, evt[k], k)
                elif key == 'GMD':
